...
def run(save_folder=None):
    ...
    # uncomment for CG run, numerically unstable.
    #newton_cg = NewtonCGAlternatingSteps(diff_L1, acc=acc, alpha0=alpha0, eH=eH, eg=eg, e_act=e_act, rho=rho_cg, max_iters=max_iters, verbose=verbose, folder=folder, device=device, oracle_termination=oracle_termination, ops_weighting=ops_weighting, max_oracles=max_oracles)
    #z_cg_alt, flag_cg_alt = newton_cg.run(z0)

    # Recover the paramters and record the accuracy/sparsity of the solutions 
    methods = [newton_mr, pg_momentum, proj_grad]

    # diffable objectives converted back.
    x_mr = l1.gen_x(z_mr, d)
    x_pg = l1.gen_x(z_pg, d)
    sols = [x_mr, x_pgm, x_pg]
