...
class NewtonCGAlternatingSteps(OptimizerNonNeg):
    ...
    def step(self) -> bool:
        ...
        if self.num_active_constraints > 0 and (self.min_gk_active < -math.sqrt(self.eg) or self.gkxk_active_norm > self.eg):
            ...
            d, self.Dtype, Hvs = cg(Hvk_inactive, gk_inactive, self.eH, self.acc, device=self.device)

            if self.Dtype == "NPC": 
                d = d/torch.linalg.norm(d)
                Hd = Hvk_inactive(d)
                # No need to count this Hessian vector product as it is technically already computed by cg. I just haven't returned it as it will make cg more complex.
                dHd = torch.dot(d, Hd)
