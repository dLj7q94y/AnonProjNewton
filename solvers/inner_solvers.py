...
def cg(Hv, g, eps, acc, M = 0, device=None):
    ...
    while k < max_iters:
        ...
        y = y + alpha*p
        # Y.append(y) 
        Hby = Hby + alpha*Hbp # Hy_{j+1} = Hy_j + \alpha H p_j
        r = r + alpha*Hbp
        ...
        normHp  = torch.linalg.norm(Hbp - 2*eps*p)
        normHy = torch.linalg.norm(Hby - 2*eps*y)
        normHr = torch.linalg.norm(Hbr - 2*eps*r)
